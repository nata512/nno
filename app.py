...
def signup():
    if request.method == "POST":
        ...
        if User.query.filter_by(username=username).first():
            flash('A user with the same name already exists.', 'danger')
        ...
        login_user(new_user)
        flash('registration was successful you are logged in', 'success')
        return redirect(url_for('index'))
...
def complete_checkout(book_id):
    ...
    flash(f"book purchase '{book.title}' was successful! adress: {address}.", "success")
...
def add_to_cart(book_id):
    ...
    flash('you added book to basket', 'success')
    return redirect(url_for('index'))
...
def delete_account():
    ...
    flash(f"our account {username} was deleted.", "success")
